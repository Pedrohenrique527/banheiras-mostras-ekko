...
type NavKey = "overview" | "materials" | "report" | "history" | "chat" | "users" | "about";
type SystemData = {
  materials: Material[];
  movements: Movement[];
  auditLogs: AuditLog[];
...
function ProductReport({ data }: { data: SystemData }) {
  const [status, setStatus] = useState("Todos");
  const [location, setLocation] = useState("Todos");
  const allProducts = useMemo(() => [
    ...data.materials,
    ...data.removedMaterials.map((item) => ({ ...item, status: "Removido" })),
  ], [data.materials, data.removedMaterials]);
  const statuses = [...new Set(allProducts.map((item) => item.status))];
  const rows = allProducts.filter((item) =>
    (status === "Todos" || item.status === status) &&
    (location === "Todos" || item.current_location === location),
  );

  function downloadCsv() {
    const headers = ["Nome", "Código", "Categoria", "Marca", "Modelo", "Status", "Local atual", "Data de entrada", "Última alteração", "Responsável", "Nota fiscal"];
    const values = rows.map((item) => [item.name, item.code, item.category, item.brand || "", item.model || "", item.status, item.current_location, item.entry_date, item.updated_at, item.last_changed_by || item.registered_by, item.invoice_number || ""]);
    const csv = [headers, ...values].map((line) => line.map((value) => `"${String(value ?? "").replace(/"/g, '""')}"`).join(";")).join("\n");
    const blob = new Blob(["\\ufeff" + csv], { type: "text/csv;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `relatorio-produtos-${new Date().toISOString().slice(0, 10)}.csv`;
    link.click();
  ...
  return (
    <section className="report-page">
      ...
      <div className="surface report-table-wrap"><div className="report-table"><div className="report-head"><span>Produto</span><span>Código / categoria</span><span>Local atual</span><span>Status</span><span>Última alteração</span></div>{rows.map((item) => <div className="report-row" key={item.id}><span><strong>{item.name}</strong><small>{item.brand}{item.model ? ` · ${item.model}` : ""}</small></span><span><strong>{item.code}</strong><small>{item.category}</small></span><span>{item.current_location}</span><span><Status value={item.status} /></span><span><strong>{formatDate(item.updated_at)}</strong><small>{item.last_changed_by || item.registered_by}</small></span></div>)}{rows.length === 0 && <div className="report-empty">Nenhum produto corresponde aos filtros escolhidos.</div>}</div></div>
      <p className="report-footnote">Gerado em {formatDate(new Date().toISOString(), true)} · O arquivo CSV pode ser aberto no Excel. Para PDF, escolha “Salvar como PDF” na janela de impressão.</p>
    </section>
  );
}
        ...
        <div className="page-content">
          ...
          {loading && data.materials.length === 0 ? <div className="loading"><span /><strong>Organizando os produtos...</strong></div> : (
            <>
              ...
              {nav === "report" && <ProductReport data={data} />}
              ...
              {nav === "users" && <Users data={data} user={user} onNew={() => setNewUser(true)} onEdit={setAccessUser} />}
              {nav === "about" && <About />}
            </>
          )}
        </div>
